...
async function getCollection(auth, collectionId) {
  ...
  return axios.get(`https://api.getguru.com/api/v1/collections/` + collectionId, {
    auth: auth
  })
}
...
async function apiSendStandardCard(auth, collectionId, title, tagValue, teamId, tagCategoryName,
  verificationInterval, verificationEmail, verificationFirstName, verificationLastName, content) {
  ...
  if (process.env.GURU_CARD_YAML) {
    try {
      apiSearchCardByTagValueAndCategoryName(auth, process.env.GURU_COLLECTION_ID, tagValue,
        tagCategoryName, content).then(response => {
          ...
          if (response.data.length >= 1) {
            try {
              ...
              apiUpdateStandardCardById(auth, process.env.GURU_COLLECTION_ID, title, response.data[0].id,
                response.data[0].tags, verificationInterval, verificationEmail, verificationFirstName,
                verificationLastName, content).then(response => {
            ...
            try {
              apiCreateTagByCategoryId(auth, tagValue, teamId, tagCategoryName).then(response => {
                try {
                  getTagCategoryId(response.data, tagCategoryName).then(tagCategoryId => {
                    ...
                    try {
                      ...
                      axios.post(`https://api.getguru.com/api/v1/teams/${teamId}/tagcategories/tags/`,
                        tagData, headers).then(response => {
                          ...
                          try {
                            return axios.post(`https://api.getguru.com/api/v1/facts/extended`, cardData,
...
async function getTagCategoryId(data, tagCategoryName) {
  console.log(`Getting Tag Category Id`, data)
...
async function apiCreateTagByCategoryId(auth, teamId) {
  ...
  try {
    return axios.get(`https://api.getguru.com/api/v1/teams/${teamId}/tagcategories`, {
      auth: auth
    })
  } catch (error) {
...
async function apiUpdateStandardCardById(auth, collectionId, title, id, tags, verificationInterval,
  verificationEmail, verificationFirstName, verificationLastName, content) {
...
function processExternalCollection(auth) {
  ...
  if (process.env.GURU_CARD_YAML) {
    ...
    for (let cardFilename in cardConfigs) try {
      ...
      console.log(
        `Writing ${cardFilename.replace(/\.md$/gi, '')} to ${tmpCardsDir}/${tmpfileBase}.yaml`);
      ...
      if (!cardConfig.ExternalUrl) {
        cardConfig.ExternalUrl =
          `https://github.com/${process.env.GITHUB_REPOSITORY}/blob/master/${cardFilename}`
      }
...
function processStandardCollection(auth) {
  if (process.env.GURU_CARD_DIR) {
    ...
    let cardConfigs = yaml.parse(fs.readFileSync(process.env.GURU_CARD_YAML, 'utf8'));
    for (let cardFilename in cardConfigs) try {
      apiSendStandardCard(auth, process.env.GURU_COLLECTION_ID, cardConfigs[cardFilename].Title,
        cardConfigs[cardFilename].UniqueTagValue, cardConfigs[cardFilename].TeamId, cardConfigs[
          cardFilename].TagCategoryName, cardConfigs[cardFilename].VerificationInterval, cardConfigs[
            cardFilename].VerificationEmail, cardConfigs[cardFilename].VerificationFirstName,
        cardConfigs[cardFilename].VerificationLastName, fs.readFileSync(path.resolve(
          `${cardFilename}`), "utf8")).then(response => {
...
try {
  ...
  getCollection(auth, process.env.GURU_COLLECTION_ID).then(response => {
    console.log(
